...
Prerequisite reading: [GETTING-STARTED.md](./GETTING-STARTED.md) for the two
...
- [`apps/examples/deploy`](../apps/examples/deploy) — Vercel-style deploy dashboard: project grid, switcher, sparklines.
- [`apps/examples/pulse`](../apps/examples/pulse) — Datadog-style monitoring console: `LineChart`, `Sparkline`.
...
`scripts/checks/export-collisions.test.ts` fails CI on a **new** react↔charts collision, and
caps the icons overlap below so it cannot grow unnoticed.

<!-- BEGIN:icon-collisions -->
<!-- Generated by scripts/docs-md/collisions.ts — run `pnpm collisions:generate`. -->

These 20 names exist in **both** `@cascivo/icons` and a component package
...
| --- | --- |
| `BarChart` | `import { BarChart as BarChartIcon } from '@cascivo/icons'` |
| `Calendar` | `import { Calendar as CalendarIcon } from '@cascivo/icons'` |
| `Code` | `import { Code as CodeIcon } from '@cascivo/icons'` |
| `Column` | `import { Column as ColumnIcon } from '@cascivo/icons'` |
| `Filter` | `import { Filter as FilterIcon } from '@cascivo/icons'` |
| `Gauge` | `import { Gauge as GaugeIcon } from '@cascivo/icons'` |
| `Glyph` | `import { Glyph as GlyphIcon } from '@cascivo/icons'` |
| `Grid` | `import { Grid as GridIcon } from '@cascivo/icons'` |
| `Heatmap` | `import { Heatmap as HeatmapIcon } from '@cascivo/icons'` |
| `Image` | `import { Image as ImageIcon } from '@cascivo/icons'` |
| `Link` | `import { Link as LinkIcon } from '@cascivo/icons'` |
| `List` | `import { List as ListIcon } from '@cascivo/icons'` |
| `Menu` | `import { Menu as MenuIcon } from '@cascivo/icons'` |
| `PieChart` | `import { PieChart as PieChartIcon } from '@cascivo/icons'` |
| `Radar` | `import { Radar as RadarIcon } from '@cascivo/icons'` |
| `Radio` | `import { Radio as RadioIcon } from '@cascivo/icons'` |
| `Search` | `import { Search as SearchIcon } from '@cascivo/icons'` |
| `Steps` | `import { Steps as StepsIcon } from '@cascivo/icons'` |
| `Tag` | `import { Tag as TagIcon } from '@cascivo/icons'` |
| `User` | `import { User as UserIcon } from '@cascivo/icons'` |

<!-- END:icon-collisions -->

**Guessing a familiar name is fine.** `@cascivo/icons` exports the common aliases from other
sets as real names, so `Rocket` (→ `Spaceship`), `LayoutDashboard` (→ `Dashboard`),
`MagnifyingGlass`, `Gear`, `Bolt` and ~35 more resolve without a lookup. The exceptions are
names already claimed elsewhere — `Kbd` is a component, `LineChart` is a chart, `Box`,
`Delete` and `Trash2` are icons in their own right — which stay searchable in the icon
gallery but are deliberately not exported as aliases.
