...
export const meta: ComponentMeta = {
  ...
  props: [
    {
      ...
      type: 'string',
      required: false,
      description: 'Accessible label for the list table.',
    },
    {
      name: 'items',
  ...
  examples: [
    {
      title: 'Static',
      code: '<StructuredList headers={["Name", "Role"]} items={[{ id: "a", cells: ["Ada", "Engineer"] }]} />',
    },
    {
      title: 'Selectable',
      code: '<StructuredList selectable defaultValue="a" items={[{ id: "a", cells: ["Ada"] }, { id: "b", cells: ["Grace"] }]} onSelect={(id) => set(id)} />',
    },
  ],
    ...
    related: [
      {
        ...
        reason:
          'DataTable adds sorting, filtering, and pagination; StructuredList stays static and lightweight',
      },
      {
        name: 'List',
        relationship: 'alternative',
        reason:
          'List is single-column vertical content; StructuredList aligns multiple cells per row',
      },
    ],
    a11yRationale:
    ...
    flexibility: [
      {
        area: 'selectable',
        level: 'flexible',
        note: 'Toggle selectable based on whether the list is interactive or purely presentational',
      },
      {
        area: 'roles',
        level: 'strict',
        note: 'Static uses table semantics; selectable uses radiogroup — do not mix',
      },
    ],
  },
}
