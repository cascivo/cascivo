...
The current pairs are in the [compatibility table](/docs/compatibility.md#package-compatibility),
...
requests**, and every answer is scoped to the versions in _your_ lockfile rather than to
...
| **B. Prebuilt (`@cascivo/react`)** | A normal npm dependency, all 198 components        | You just want to _use_ the system; upgrades via `pnpm up` |
...
[USING-WITH-STRICT-ESLINT.md](/docs/using-with-strict-eslint.md) §1 for what turning
the rule off costs.
...
Babel plugin), so adding it is always safe. You do _not_ need it to pass signals into a
