...
<Modal>
  ...
  <Button onClick={onConfirm} style={{ background: '#ef4444' }}>
    Delete
  </Button>
...
3. Finding counts are computed by running the _A outputs_ through `cascade audit --ai` and recording actual tool output. The B outputs were verified as audit-clean.
