...
- [ ] Virtualized rendering for large row counts (own implementation, signal-driven scroll position; no react-window dependency).
...
- [ ] Verify: a docs demo with 10,000 rows sorts/filters with no perceptible jank.
...
### Milestone 6.5 — Quality gates expansion
...
| 2 | Phase 1.1 + 1.4 (shell, nav, display) | M | Factory-friendly, unblocks layouts |
| 3 | Phase 3 (i18n + storage) | M | Everything downstream consumes them |
| 4 | Phase 1.2 DataTable + 1.3 forms + 1.5 cmd-k | L | Flagship DX components, need i18n strings |
| 5 | Phase 2 layouts + blocks | M | Needs 1.1; unblocks JSON renderer demos |
| 6 | Phase 4.1 + 4.2 charts | L | Longest pole — start engine as soon as DataTable ships |
| 7 | Phase 5 JSON renderer → codegen | L | Needs component breadth + layouts |
| 8 | Phase 6.2–6.5 DX/AI completion | M | Gates and skills verify the whole platform |
| 9 | Phase 4.3 chart parity tail | M | Demand-driven |
