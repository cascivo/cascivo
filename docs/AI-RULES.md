...
1. Local state -> `useSignal(initial)`; derived -> `useComputed(fn)`. Never `useState`.
   ...
   > [USING-WITH-STRICT-ESLINT.md](./USING-WITH-STRICT-ESLINT.md).
...
Full guide: [USING-WITH-A-ROUTER.md](./USING-WITH-A-ROUTER.md).
...
- **`data-cascivo-*` style hooks are the sanctioned exception**, and they are public API:
  ...
  not stamp. See [STYLING-INTERNALS.md](./STYLING-INTERNALS.md).
...
the CSS. The names live in [`@cascivo/tokens/style-contract.json`](./TOKENS.md).
...
See [MACHINE-MODE.md](./MACHINE-MODE.md).
...
- [USING-WITH-A-ROUTER.md](./USING-WITH-A-ROUTER.md) — `setLinkComponent` vs `asChild`.
...
- [MACHINE-MODE.md](./MACHINE-MODE.md) — a rendered UI as a Markdown document.
