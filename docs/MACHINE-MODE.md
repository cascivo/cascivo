...
To show the UI **and** its text — the side-by-side on cascivo.com — use the hook `<TextView>`
is built on, against a container of your own:
...
It sits on a subpath rather than the root export because it renders to a string, and a root
