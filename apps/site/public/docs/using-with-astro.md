...
with `node` active, so its _server_ module graph got the CSS-free build — and Astro collects
...
`vite.ssr.noExternal` — what [`USING-WITH-VITE-SSR.md`](/docs/using-with-vite-ssr.md)
...
- **The fix is verified in CI, on a vanilla config.**
  [`apps/examples/astro-islands`](https://github.com/cascivo/cascivo/tree/main/apps/examples/astro-islands) builds a real Astro app
