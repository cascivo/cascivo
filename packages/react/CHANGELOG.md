...
- 265e976: Fix SSR'd Astro islands rendering unstyled, and the same latent bug in every Vite-based SSR
  framework.
...
- 2050fe5: `TreeView` could become entirely untabbable, and its collapsed branches were rendered anyway.
...
- 2050fe5: The published chunks are actually minified now, which takes the whole library from 102.4 KB
...
- 2050fe5: `scripts/tsdoc/generate.ts` now searches only inside `export interface …Props` blocks, which
...
- 58c165e: `DataTable` handles a million rows: reachable to the last row, measured row height,
...
- f1c8292: Two invariants that were documented and not enforced are now guarded.
...
- b16cb6c: Fix: server-rendered components lost their CSS under RSC, forcing every SSR app onto the
