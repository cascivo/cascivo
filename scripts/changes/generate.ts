...
function main(): void {
  ...
  writeFileSync(CHANGELOG_PATH, buildRootChangelog(packages), 'utf8')
  console.log('Wrote root CHANGELOG.md index')
}

/** Root CHANGELOG: an index over the per-package changesets CHANGELOGs. */
function buildRootChangelog(packages: PackageChanges[]): string {
  const lines: string[] = [
  ...
  lines.push('', 'See [docs/UPGRADING.md](docs/UPGRADING.md) for upgrade guidance.', '')
  return lines.join('\n')
}
