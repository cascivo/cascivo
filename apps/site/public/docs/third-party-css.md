...
cascivo ships every rule inside a [cascade layer](/docs/css-layers-pitfall.md). The whole
...
- [CSS-LAYERS-PITFALL.md](/docs/css-layers-pitfall.md) — the canonical layer order and the
  `cascivo.override` escape hatch.
- [USING-WITH-TAILWIND.md](/docs/using-with-tailwind.md) — the same layering technique
  applied to Tailwind's utilities.
- [CHART-LIBRARIES.md](https://github.com/cascivo/cascivo/blob/main/docs/CHART-LIBRARIES.md) — if you adopt Chart.js or ECharts anyway,
  tame their CSS with the recipe above.
- [TROUBLESHOOTING.md](/docs/troubleshooting.md) — symptom → cause → fix for unstyled UI.
