...
// copies each demo's built `dist/` into `apps/docs/dist/demos/<slug>/`, so a
...
//
// The landing itself must already be built (so `apps/docs/dist/` exists);
...
for (const [slug, pkg] of Object.entries(DEMOS)) {
  ...
  console.log(`[assemble-demos] ${slug} → apps/docs/dist/demos/${slug}/`)
}
