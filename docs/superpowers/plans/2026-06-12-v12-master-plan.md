...
| 6   | CSS contract test (repo-wide, in components package): parse every `*.module.css`; assert zero `!important`; assert visual-property values (color/background/border/box-shadow/border-radius/margin/padding/gap/font-_) are `var(--cascivo-_)`, `var(--\_\*)`, or structural keywords (none/transparent/inherit/currentColor/0); maintain an explicit, commented allowlist file for justified exceptions | Pain #2 becomes a gate; allowlist keeps it honest rather than rubber-stamped |
