...
**The shipped answer** is [`ThemeProvider`](../packages/core/src/theme.tsx), now exported
...
covered by semver — see the [stability contract](./UPGRADING.md#the-stability-contract) for
...
stability promise; the [coverage table](./UPGRADING.md#which-packages-are-covered) lists
...
- **A published deprecation policy** — a component marked `deprecated` in its manifest keeps
  working and is signposted at *discovery* time: `cascivo list`, `cascivo search`, `cascivo
