...
- Hero: H1 "ships like *shadcn*, performs like a *signal*, thinks like an *agent*" — names a
...
our console is *re-themeable per region* (data-theme scoping), *counted* (live Profiler
counter), and *agent-addressable* (a slice of it renders from visible JSON via CascadeView).
...
2. **AppShell in the console**: AppShell imports `@cascade-ui/storage` and manages viewport
   ...
   may need a *framed* composition (SideNav + header inside a Card-like bordered container)
