...
| Project-card grid | `Card` + `Badge` (framework/status) + `RelativeTime` (last deploy) | `card`, `badge`, `relative-time` | Lay the grid out with CSS grid/flex directly — no special "card grid" component is needed. |
