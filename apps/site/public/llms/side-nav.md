...
| `header`             | `ReactNode \| ((ctx: SideNavSlotContext) => ReactNode)` | no       | —                     | Content rendered above the items, inside the item padding context (e.g. app-context pickers). Pass a `({ collapsed }) => …` function to shrink it to a rail icon.                                                                                                                                         |
| `footer`             | `ReactNode \| ((ctx: SideNavSlotContext) => ReactNode)` | no       | —                     | Content rendered above the collapse toggle, inside the item padding context so it lines up with the nav items (e.g. version string, user info). Pass a `({ collapsed }) => …` function to shrink it to a rail icon.                                                                                       |
