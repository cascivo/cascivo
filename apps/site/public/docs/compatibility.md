...
| Ghost (Handlebars themes)   | ⚠️ Tokens/themes only | Themes are Handlebars rendered server-side with no JS framework layer, so React components cannot mount. The token + theme CSS works once flattened (bare `@import`s, no build step in Ghost). See [`USING-WITH-GHOST.md`](/docs/using-with-ghost.md).                                                                                                                                                  |
