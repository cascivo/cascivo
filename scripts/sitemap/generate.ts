 ...
 * stays green; run as part of `pnpm regen`. `<lastmod>` is only emitted for
 * blog posts, whose dates are static hand-authored fields — every other
 * route omits it rather than deriving it from `git log`, since a source file
 * driving a route's content can be edited in the very same commit that
 * regenerates this file, making a git-log-derived date here perpetually one
 * commit stale relative to the date git will report once that commit lands.
 */
...
const entries = [
  ...
  ...categoryRoutes.map((cat) => urlEntry(`${SITE_URL}/docs/categories/${cat}`, '0.7')),
  ...components.map((c) => urlEntry(`${SITE_URL}/docs/components/${c.name}`, '0.6')),
