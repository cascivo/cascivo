...
export const meta: ComponentMeta = {
  ...
  props: [
    ...
    },
    {
      name: 'searchable',
      description:
        'When true the field is a text input that filters the list as the user types (the APG editable combobox). When false it is a button that opens the list, and type-to-select jumps to a matching option (the APG select-only combobox).',
      type: 'boolean',
      required: false,
      default: 'true',
    },
    {
      name: 'loading',
      description:
        'When true, the list reports itself as busy and shows a loading row instead of the empty message.',
      type: 'boolean',
      required: false,
      default: 'false',
    },
    ...
    {
      name: 'onOpenChange',
      description: 'Called when the listbox opens or closes.',
  ...
  examples: [
    {
      ...
      code: `<Combobox options={[{value:'de',label:'Germany',group:'Europe'},{value:'jp',label:'Japan',group:'Asia'}]} />`,
      description: 'Options carrying a group render under a labelled role="group" heading.',
    },
    {
      title: 'Remote search',
      code: `<Combobox options={results} loading={pending} onSearchChange={search} filter={() => true} />`,
      description:
        'filter={() => true} hands filtering to the server; loading marks the list busy between keystroke and response.',
    },
    {
      title: 'Select-only',
      code: `<Combobox options={options} searchable={false} />`,
      description:
    ...
    related: [
      {
        ...
        reason: 'Use for short lists that do not need type-to-filter',
      },
      {
        name: 'MultiSelect',
        relationship: 'alternative',
        reason: 'Use when more than one value can be selected',
      },
      {
        name: 'CommandMenu',
        relationship: 'alternative',
        reason: 'Use the Cmd+K palette for command/navigation search rather than value selection',
      },
    ],
    a11yRationale:
    ...
    flexibility: [
      {
        area: 'searchable',
        level: 'flexible',
        note: 'Filtering can be toggled off for short lists via searchable={false}',
      },
      {
