...
| **B. Prebuilt (`@cascivo/react`)** | A normal npm dependency, all 193 components      | You just want to *use* the system; upgrades via `pnpm up`             |
...
> decision table is in [THEMING.md](./THEMING.md#where-does-the-attribute-go--html-or-an-element).
...
[BENCHMARKS.md](./BENCHMARKS.md) for min+gzip totals vs shadcn/ui and Carbon plus the
...
> [TROUBLESHOOTING.md](./TROUBLESHOOTING.md#tsc-fails-on-the-css-import-cannot-find-module-cascivoreactstylescss-ts2307--ts2882).
