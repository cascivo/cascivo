...
their scale. That is a real win at Meta's size. cascivo gets the same *shape* of result from
...
| `stylex.createTheme(vars, { … })` | `[data-theme="…"]` on any element — see [THEMING.md](./THEMING.md) |
...
- [AI-RULES.md](./AI-RULES.md) — the layer contract, the override ladder, and the
  no-styling-at-a-distance principle.
- [STYLING-INTERNALS.md](./STYLING-INTERNALS.md) — the `data-cascivo-*` hooks.
- [TOKENS.md](./TOKENS.md) — the full token catalog.
- [CSS-LAYERS-PITFALL.md](./CSS-LAYERS-PITFALL.md) — running cascivo's layers next to
  another system's.
