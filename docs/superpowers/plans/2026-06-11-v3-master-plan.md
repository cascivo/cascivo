...
**Goal:** Implement Roadmap v3 — from application platform to the design system you _prefer_ — in
...
`factory-backlog.json`. AI components ship as a new published package `@cascivo/ai` (not
...
4. **Two new themes** ship as separate CSS files: `packages/themes/src/flat.css` and
   ...
   both as archetypes.
5. **AI components published in `@cascivo/ai`** — a new npm package, not copy-paste registry
   items. Depends on `@cascivo/core`, `@cascivo/i18n`, `@cascivo/tokens`.
...
- **apps/docs has /ai page;** `llms.txt` and per-component markdown exist; four Claude Code skills
  exist in `skills/`.
- **Token layer:** `--cascivo-motion-*` semantic tokens exist. `--cascivo-radius-*` tokens exist
...
| 5   | Semantic radius split (`--cascivo-radius-control/surface/indicator`) requires updating every component CSS file    | Automated sed pass in T4 (script-assisted migration); visual regression baseline re-recorded deliberately in a dedicated PR.                                                                                 | 4       |
...
| 7   | `minimal.css` near-invisible borders — WCAG distinction: decoration vs. information                                | Borders that convey state (error, focus) remain high-contrast; decorative divider borders are the "invisible" ones. Contrast-check script distinguishes by token name (`--cascivo-border-subtle` is exempt). | 5       |
| 8   | `@cascivo/ai` streaming animation uses `requestAnimationFrame` — must clean up in `useSignalEffect` return         | `useSignalEffect` cleanup pattern is the standard cascade idiom; explicitly shown in T6 StreamingText implementation.                                                                                        | 6       |
| 9   | AiChat markdown rendering: full remark vs. minimal subset                                                          | Minimal subset only (bold/italic/code/fences/links/lists) — no full remark pipeline. Recorded decision; bundle budget gate enforces it (< 20 KB gzip for `@cascivo/ai`).                                     | 6       |
| 10  | ErrorBoundary home: `@cascivo/core` vs. copy-paste component                                                       | Ships in `@cascivo/core` (no styling of its own; not a registry copy-paste item). Exported from `@cascivo/react` for convenience.                                                                            | 3       |
...
| 12  | ContextMenu CSS anchor at pointer position — anchor positioning spec doesn't support pointer coords natively       | `--cascivo-context-x` / `--cascivo-context-y` CSS custom properties set via `onContextMenu` handler (single DOM write per right-click); CSS reads them via `left: var(--cascivo-context-x)`.                 | 1       |
| 13  | Dark factory integrate step must add to `@cascivo/react` exports — not currently enforced                          | Verify factory skill SKILL.md already has the export step (added in v2 T1 Task 2); if missing, re-add before queuing T2 items.                                                                               | 2       |
...
| 6   | **Terminal + AiChat + StreamingText** — new `@cascivo/ai` package with StreamingText, AiLabel, Terminal (TerminalBlock), AiChat; ai-chat-page block; docs /ai page update                                     | Phase 3 Milestones 3.1 + 3.2 + 3.3 | L    | `2026-06-11-v3-tranche-6.md` |
...
- **T2:** All 8 input components at `review`/`done` in factory backlog; exported from
  `@cascivo/react`; MultiSelect's Popover integration confirmed working (Playwright smoke test).
...
- **T6:** `@cascivo/ai` builds and publishes; AiChat streaming benchmark shows 60fps with zero
...
- **T7:** All DoD gates from ROADMAP-V3 Phase 5 enforced in CI; bundle budgets green
  (`@cascivo/react` < 50 KB gzip, `@cascivo/ai` < 20 KB gzip, `@cascivo/charts` < 80 KB
