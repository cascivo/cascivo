...
- Landing deps already include `@cascade-ui/charts`, `@cascade-ui/icons` (+ vite alias),
  `@cascade-ui/components`, `@cascade-ui/render`. Playwright + @axe-core/playwright run
...
`packages/react/src/index.ts`; chrome strings from `@cascade-ui/i18n` builtin catalog with
...
7. **Chart fallbacks**: `@cascade-ui/charts` components render a `<table>` fallback for
...
13. **Registry/readme/llms drift**: 12 new components touch every generated artifact;
    ...
    CI.
14. **`VisuallyHidden` name collision**: `@cascade-ui/core` already exports a
