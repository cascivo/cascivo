...
| `tabIndex`  | `number`                                                                                          | No       | —            | Tab-order position of the trigger. Composite widgets that manage their own focus (a DataTable in grid keyboard mode) pass -1 so the menu is reachable by Enter on its cell without being an extra Tab stop. |
