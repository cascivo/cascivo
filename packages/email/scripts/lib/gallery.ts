...
/**
 * One message that uses nearly every primitive at once.
 *
 * The gallery above answers "what is each piece"; this answers "what do they add up to",
 * which is the question a landing page has about ten seconds to answer. It is a real render
 * like everything else here — the same primitives, the same themes, the bytes an adopter
 * would send.
 *
 * `Markdown` is left out deliberately: its whole point is prose that arrives at send time,
 * and a canned string in a showcase would misrepresent it. `Style` is present but invisible —
 * `Container` declares the responsive width rule, which is exactly the case `Style` exists for.
 */
export const SHOWCASE = {
  subject: 'Your February summary',
  el: h(
    Html,
    null,
    h(Head, { title: 'Your February summary' }),
    h(
      Body,
      null,
      h(Preview, null, 'One message, every primitive — and it still renders in Outlook'),
      h(
        Container,
        null,
        h(
          Section,
          { padding: 24, background: '#1f2937', align: 'center' },
          h(Img, { src: SAMPLE_IMAGE, alt: 'Acme', width: 48 }),
        ),
        h(
          Section,
          { padding: 24 },
          h(Heading, { level: 1 }, 'Your February summary'),
          h(Spacer, { height: 12 }),
          h(
            Text,
            { variant: 'muted' },
            'Everything below is one React tree rendered to table-based, inline-styled HTML.',
          ),
          h(Spacer, { height: 20 }),
          h(Alert, { tone: 'info', title: 'Heads up' }, 'Your plan renews on 1 March.'),
          h(Spacer, { height: 20 }),
          h(
            Card,
            { padding: 20 },
            h(
              Row,
              null,
              h(Column, { width: '60%' }, h(Heading, { level: 3 }, 'Pro plan')),
              h(Column, { width: '40%', align: 'right' }, h(Badge, { tone: 'success' }, 'Paid')),
            ),
            h(Spacer, { height: 12 }),
            h(Text, { variant: 'muted', size: '14px' }, '12 seats · €348 billed monthly'),
            h(Spacer, { height: 16 }),
            h(Button, { href: 'https://example.com/billing', block: true }, 'View the invoice'),
          ),
          h(Hr, { spacing: 24 }),
          h(Heading, { level: 3 }, 'What shipped'),
          h(Spacer, { height: 12 }),
          h(List, { items: ['Per-seat billing, prorated', 'CSV export', 'Audit log'] }),
          h(Spacer, { height: 24 }),
          h(
            Row,
            null,
            h(
              Column,
              { width: '50%', stack: true },
              h(Text, { size: '14px', variant: 'muted' }, 'Questions?'),
            ),
            h(
              Column,
              { width: '50%', align: 'right', stack: true },
              h(
                Button,
                { href: 'https://example.com/support', variant: 'secondary' },
                'Talk to us',
              ),
            ),
          ),
